...
1. **딥러닝(BERT/LLM)에 욕심내지 않기**:
   ...
   * 기초가 없는 상태에서 딥러닝 임베딩을 시도하면 환경 설정과 GPU 에러 해결에만 3주 이상 낭비될 수 있습니다. `TfidfVectorizer`와 `Pandas 조건문`만으로도 충분히 훌륭한 결과물이 나옵니다.

2. **함수 단위로 역할 쪼개기**:

   * 파이썬 함수(`recommend_games(...)`)를 그대로 FastAPI 내부에서 호출하기만 하면 되므로, 코드 충돌이 적습니다.